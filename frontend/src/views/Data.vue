...
<template>
  <div class="min-h-screen bg-gray-50">
    <div class="mx-auto max-w-screen-lg">
      <!-- 搜索 -->
      <div class="sticky top-14 z-40 border-b border-gray-100 bg-white px-4 py-3">
        <input
          v-model="searchQuery"
          type="text"
          placeholder="搜索球队..."
          class="w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-2 text-sm focus:border-transparent focus:outline-none focus:ring-2 focus:ring-green-500"
        />
      </div>
      ...
      <div class="px-4 py-4">
        ...
        <div class="mb-4 flex gap-3">
          <button
            class="flex-1 rounded-xl border border-gray-100 bg-white p-3 text-center transition-shadow hover:shadow-md"
            @click="goRankings"
          >
          ...
          >
            ...
            <span class="text-xs text-gray-600">{{
              filterMode === 'group' ? '按大洲' : '按小组'
            }}</span>
          </button>
        </div>

        <!-- 筛选 Tab -->
        <div class="scrollbar-hide mb-4 flex gap-1 overflow-x-auto">
          <template v-if="filterMode === 'group'">
            <button
              class="flex-shrink-0 rounded-full px-3 py-1 text-xs transition-colors"
          ...
          <template v-else>
            <button
              class="flex-shrink-0 rounded-full px-3 py-1 text-xs transition-colors"
              :class="
                activeConfederation === '' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-500'
              "
        ...
        <div v-else-if="filteredTeams.length > 0" class="grid grid-cols-2 gap-3">
          <div
            ...
            :key="team.id"
            class="cursor-pointer rounded-xl border border-gray-100 bg-white p-4 transition-shadow hover:shadow-md"
